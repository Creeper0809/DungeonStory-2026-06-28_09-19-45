...
public static class CharacterAiBehaviorDesignerGraphBuilder
{
    ...
    public static bool EnsureVisualGraph(ExternalBehaviorTree externalBehavior)
    {
        ...
        Sequence critical = CreateTask<Sequence>(id++, "Critical", string.Empty, new Vector2(-620f, 20f));
        HasCriticalState hasCritical = CreateTask<HasCriticalState>(id++, "Has Critical?", string.Empty, new Vector2(-720f, 120f));
        RunCriticalState runCritical = CreateTask<RunCriticalState>(id++, "Run Critical", string.Empty, new Vector2(-520f, 120f));
        AddChildren(critical, hasCritical, runCritical);

        Selector macro = CreateTask<Selector>(id++, "Macro Goals", string.Empty, new Vector2(-160f, 20f));
        AddChildren(
            macro,
        ...
        Sequence continueCurrent = CreateContinueCurrentBranch(ref id, new Vector2(300f, 20f));
        Sequence stopCurrent = CreateStopCurrentBranch(ref id, new Vector2(580f, 20f));

        PrioritySelector normal = CreateTask<PrioritySelector>(id++, "Normal Routine", string.Empty, new Vector2(980f, 20f));
        AddChildren(
            normal,
            CreateSurvivalNeedsRoutine(ref id, new Vector2(700f, 150f)),
            CreateDutyWorkRoutine(ref id, new Vector2(1480f, 150f)),
            CreateLeisureVisitRoutine(ref id, new Vector2(700f, 530f)),
            CreateIdleRoutine(ref id, new Vector2(1220f, 530f)));

        AddChildren(root, critical, macro, continueCurrent, stopCurrent, normal);
